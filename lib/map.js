...
var map = L.map("mapDIV", {
 ...
 /*   var infoNJC = L.control();

    infoNJC.onAdd = function (map) {
        this._div = L.DomUtil.create('div', 'info');
        this.update();
        return this._div;
    };

    infoNJC.update = function (props) {
        this._div.innerHTML = '<h4>US Population Density</h4>' 
            +  (props ? '<b>NJ Congressional District : ' 
            + props.district 
            + '</b><br />' 
            + props.density 
            + ' people / mi<sup>2</sup>'
...
//});
//$.getJSON("https://arcgis.dvrpc.org/portal/rest/services/Boundaries/CountyBoundaries/FeatureServer/0/query?where=query?where=1=1&outFields=*&outSR=4326&f=pgeojson", function (data) {
//    DVRPC.addData(data);
//});
//DVRPC.addTo(map);
...
var layer1icon = L.OpenFreightMarkers.icon({
    ...
    layer0icon = L.OpenFreightMarkers.icon({
        layer:'layer0', titleicon:'<style="background-color:#24527F; display: inline-block; height: 5px;margin-right: 5px;width: 40px;">',title: '<span2 id="legend-icon" style="background-color: #24527F"></span2> NJ Congressional',legendMarker: 'checkbox'})
      ;
...
DVRPC.addTo(map);
    ...
    function highlightFeatureNJC(e) {
      ...
      //  infoNJC.update(layer.feature.properties);
    }
...
var layer0 = L.geoJson(null, {
  ...
  style: {stroke:true, fillColor:'#ffe8e4',color: '#f72c00', weight: 3,fill: true, clickable: true},  
     onEachFeature: function (feature, layer){  
           if (feature.properties.district[8] === "F") {
                        layer.bindLabel(feature.properties.district, { noHide: true, direction: "auto", className: "leaflet-label-RSEF" });               
                }
            else {
          layer.bindLabel(feature.properties.district, { noHide: true, direction: "auto", className: "leaflet-label-RSEU" });                      
            };
